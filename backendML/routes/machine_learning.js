...
router.get('/SearchData', auth, async (req, res) => {
	...
	let usData = await UserSearches.find({ user: userID })
	...
	// ml start here
	// get the last 3 items on the usersearches list re to our selected user

	let user_search = await UserSearches.find({ user: userID })
	...
	if (user_search.length > 0) {
		for (let index = 0; index < user_search.length; index++) {
			...
			prefProducts.push(clist.data['name']);
		}
	}

	// console.log('data', prefProducts)
	...
	let allProducts = await Products.find({
		prod_categories: ProductSelected.prod_categories,
	...
	for (let index = 0; index < trainingData.length; index++) {
		...
		if (FinalResult.length == 3) {
			...
			for (let index = 0; index < FinalResult.length; index++) {
				...
				let object = await Products.find({ $text: { $search: res } }).limit(1);
...
router.patch('/ml_products/:ProdID', auth, async (req, res) => {
	...
	let User_Prod_list = await ML_Products.find({ UserID_ML_Products: userID })
	...
	if (User_Prod_list.length > 0) {
		for (let index = 0; index < User_Prod_list.length; index++) {
			...
			prefProducts.push(clist.Selected_ML_Products['name']);
		}
	}
...
router.patch('/ml_cart/:ProdID', auth, async (req, res) => {
	...
	let user_cart_list = await ML_Cart.find({ UserID_ML_Cart: userID })
	...
	if (user_cart_list.length > 0) {
		for (let index = 0; index < user_cart_list.length; index++) {
			...
			prefProducts.push(clist.Selected_ML_Cart['name']);
		}
	}
